...
export function runBuildConfig(rootPath) {
  let configPath = path.join(rootPath, 'spider-ide', "config");
  // Vérifier si le chemin existe
  if (!fs.existsSync(configPath)) {
    // Créer le dossier config s'il n'existe pas
    fs.mkdirSync(configPath, { recursive: true });
  } else {
    // Retrieve folder names in configPath
    const folderNames = fs.readdirSync(configPath)
      .filter((name) => {
        const folderPath = path.join(configPath, name);
...
export function getConfig(rootPath, folderName) {
  const configFolderPath = path.join(rootPath, 'spider-ide', 'config', folderName);
  ...
  try {
    ...
    if (jsonFiles.length === 1) {
      ...
      console.error('Expected one JSON file, found', jsonFiles.length);
      return null;
    }
  } catch (error) {
    console.error('Error reading config folder:', error);
    return null;
  }
}
